            ...
            <DialogContent className="bg-[#121214] border-[#232326] text-[#EDEDF0]">
              ...
              <div className="space-y-4 py-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button type="button" data-testid="source-local" onClick={() => { if (!user?.limits?.local_broadcast) { toast.error("Live local broadcast isn't on your plan — upgrade to use it."); return; } setSourceLocal(true); }} className={`flex-1 h-9 rounded-sm border font-mono text-[10px] uppercase tracking-wider ${sourceLocal ? "border-[#5A67D8] text-white bg-[#5A67D8]/10" : "border-[#232326] text-[#8A8A93]"} ${!user?.limits?.local_broadcast ? "opacity-50" : ""}`}>Local file · live{!user?.limits?.local_broadcast ? " 🔒" : ""}</button>
                </div>
                {!sourceLocal ? (
